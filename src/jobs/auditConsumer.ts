  ...
  channel.consume(QUEUES.AUDIT_LOGS, async (msg: ConsumeMessage | null) => {
    ...
    try {
        ...
        try {
          await prisma.auditTrail.create({
            data: {
              ...
              timestamp: validatedEntry.timestamp ? new Date(validatedEntry.timestamp) : undefined,
            },
          });
          success = true;
          channel.ack(msg);
      ...
      if (error instanceof MessageValidationError) {
        ...
        await deadLetterMessage(QUEUES.AUDIT_LOGS, content, `Validation failed: ${error.message}`);
        channel.ack(msg);
      } else {
